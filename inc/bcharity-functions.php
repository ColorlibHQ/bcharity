...
=========================================================*/
function bcharity_opt( $id = null, $default = '' ) {
...
function bcharity_site_icon(){
	if ( ! has_site_icon() ) {
		$html = '';
		$icon_path = BCHARITY_DIR_ASSETS_URI . 'img/favicon.png';
...
=========================================================*/
function bcharity_meta( $id = '' ){
    
    $value = get_post_meta( get_the_ID(), '_bcharity_'.$id, true );
...
/*=========================================================
	Blog Date Permalink
=========================================================*/
function bcharity_blog_date_permalink(){
...
/*========================================================
	Blog Excerpt Length
========================================================*/
if ( ! function_exists( 'bcharity_excerpt_length' ) ) {
	function bcharity_excerpt_length( $limit = 30 ) {
...
==========================================================*/
if ( ! function_exists( 'bcharity_posted_comments' ) ) {
    function bcharity_posted_comments(){
        
        $comments_num = get_comments_number();
        if( comments_open() ){
            if( $comments_num == 0 ){
                $comments = esc_html__('No Comments','bcharity');
            } elseif ( $comments_num > 1 ){
                $comments= $comments_num . esc_html__(' Comments','bcharity');
            } else {
                $comments = esc_html__( '1 Comment','bcharity' );
            }
            $comments = '<i class="ti-comments"></i>'. $comments;
        } else {
            $comments = esc_html__( 'Comments are closed', 'bcharity' );
        }
...
/*===================================================
	Post embedded media
===================================================*/
function bcharity_embedded_media( $type = array() ){
...
/*===================================================
	WP post link pages
====================================================*/
function bcharity_link_pages(){
    wp_link_pages( array(
    'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'bcharity' ) . '</span>',
    'after'       => '</div>',
    'link_before' => '<span>',
    'link_after'  => '</span>',
    'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'bcharity' ) . ' </span>%',
...
/*====================================================
	Theme logo
====================================================*/
function bcharity_theme_logo( $class = '' ) {

    $siteUrl = home_url('/');
    // site logo
		
	$custom_logo_id = get_theme_mod( 'custom_logo' );
	$imageUrl = wp_get_attachment_image_src( $custom_logo_id , 'bcharity_logo_178x56' );
	
	if( !empty( $imageUrl[0] ) ){
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( $imageUrl[0] ).'" alt="bcharity logo"></a>';
	}else{
...
/*================================================
    Page Title Bar
================================================*/
function bcharity_page_titlebar() {
	if ( ! is_page_template( 'template-builder.php' ) ) {
        ...
        <section class="hero-banner d-flex align-items-center">
            <div class="container">
				...
				<?php
				if ( is_category() ) {
					single_cat_title( __('Category: ', 'bcharity') );
					...
					echo esc_html__( 'Search for: ', 'bcharity' ) . get_search_query();

				} elseif ( ! ( is_404() ) && ( is_single() ) || ( is_page() ) ) {
					echo  get_the_title();

				} elseif ( is_home() ) {
					echo esc_html__( 'Blog', 'bcharity' );

				} elseif ( is_404() ) {
					echo esc_html__( '404 error', 'bcharity' );
...
=================================================*/
function bcharity_pull_right( $id = '', $condation ){
...
/*======================================================
	Inline Background
======================================================*/
function bcharity_inline_bg_img( $bgUrl ){
    $bg = '';
...
/*======================================================
	Blog Category
======================================================*/
function bcharity_featured_post_cat(){
...
========================================================*/
function bcharity_sidebar_opt(){

    $sidebarOpt = bcharity_opt( 'bcharity_blog_layout' );
    $sidebar = '1';
...
/**================================================
	Themify Icon
 =================================================*/
function bcharity_themify_icon(){
    return[
        'cap'     => __('Icon Cap', 'bcharity'),
        'bag'     => __('Icon Bag', 'bcharity'),
        'shirt'   => __('Icon T-shirt', 'bcharity'),
        'cafe'    => __('Icon Cafe', 'bcharity'),
    ];
}
...
============================================================*/
function bcharity_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'bcharity_contact7_form_content', 10, 2 );
...
/*============================================================
	Pagination
=============================================================*/
function bcharity_blog_pagination(){
	echo '<nav class="blog-pagination justify-content-center d-flex">';
        the_posts_pagination(
            array(
                'mid_size'  => 2,
                'prev_text' => __( '<span class="ti-angle-left"></span>', 'bcharity' ),
                'next_text' => __( '<span class="ti-angle-right"></span>', 'bcharity' ),
                'screen_reader_text' => ' '
            )
        );
	echo '</nav>';
...
=============================================================*/
if( ! function_exists('bcharity_blog_single_post_navigation') ) {
	function bcharity_blog_single_post_navigation() {
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-12 nav-left flex-row d-flex justify-content-start align-items-center">
						...
						if( get_next_post_link() ){
							...
							if( has_post_thumbnail() ){
								...
								<div class="thumb">
									<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
										<?php echo get_the_post_thumbnail( absint( $nextPost->ID ), 'bcharity_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
									</a>
								</div>
								<?php
							} ?>
							...
							<div class="detials">
								<p><?php echo esc_html__( 'Prev Post', 'bcharity' ); ?></p>
								<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
					...
					<div class="col-lg-6 col-md-6 col-12 nav-right flex-row d-flex justify-content-end align-items-center">
						...
						if( get_previous_post_link() ){
							...
							?>
							<div class="detials">
								<p><?php echo esc_html__( 'Next Post', 'bcharity' ); ?></p>
							...
							<div class="thumb">
								<a href="<?php the_permalink(  absint( $prevPost->ID )  ) ?>">
									<?php echo get_the_post_thumbnail( absint( $prevPost->ID ), 'bcharity_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
								</a>
							</div>
							<?php
						} ?>
...
/*===================================================
 Bcharity Comment Template Callback
 ====================================================*/
function bcharity_comment_callback( $comment, $args, $depth ) {
				...
				<div class="desc">
					...
					<div class="d-flex justify-content-between">
						<div class="d-flex align-items-center">
							<h5 class="comment_author"><?php printf( __( '<span class="comment-author-name">%s</span> ', 'bcharity' ), get_comment_author_link() ); ?></h5>
							<p class="date"><?php printf( __('%1$s at %2$s', 'bcharity'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'bcharity' ), '  ', '' ); ?> </p>
							<?php if ( $comment->comment_approved == '0' ) : ?>
								<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'bcharity' ); ?></em>
								<br>
							<?php endif; ?>
						</div>
...
add_filter('comment_reply_link', 'bcharity_replace_reply_link_class');
function bcharity_replace_reply_link_class( $class ) {
...
===========================================================*/
function bcharity_latest_blog( $post_cat = 'inspiration', $post_order = 'DESC' ){
	
	function bcharity_get_first_catname_link() {
		$bcharity_cat = '';
		$categories = get_the_category();
		if ( ! empty( $categories ) ) {
			$bcharity_cat .= '<a href="' . esc_url( get_category_link( $categories[0]->term_id ) ) . '">' . esc_html( $categories[0]->name ) . '</a>';
		}
		return $bcharity_cat;
	}
		...
		<div class="col-lg-6">
			<div class="single_blog">
				<div class="appartment_img">
					<?php
						if( has_post_thumbnail() ){
							the_post_thumbnail( 'bcharity_latest_blog_555x443', ['alt' => get_the_title() ] );
						}
					?>
				</div>
				<div class="single_appartment_content">
					...
					<ul class="list-unstyled">
						<li><a href="<?php echo esc_url( bcharity_blog_date_permalink() )?>"> <span class="flaticon-calendar"></span> <?php the_time('F j, Y') ?> </a></li>
						<li><a href="<?php the_permalink(); ?>">  <?php echo bcharity_posted_comments();?> </a></li>
					</ul>
				</div>
			</div>
		</div>
				...
				<div class="single_blog">
					<div class="media">
						<div class="media-body align-self-center">
							<p><?php echo bcharity_get_first_catname_link()?></p>
...
/*=========================================================
    Share Button Code
===========================================================*/
function bcharity_social_sharing_buttons( $ulClass = '', $tagLine = '' ) {
 ...
 *  Flaticon Icon List
=========================================================*/
function bcharity_flaticon_list(){
    return(
        array(
